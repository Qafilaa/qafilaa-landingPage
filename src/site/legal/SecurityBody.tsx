// Generated from `Qafilaa Site v3.dc.html` (handoff 14), lines 1363-1402.
...
export function SecurityBody() {
  return (
    <article>
            ...
            <div style={{ fontFamily: '\'Space Grotesk\',sans-serif', fontSize: 'var(--qf-fs-19)', fontWeight: '500', color: '#0A6068', marginTop: '22px' }}>How we hold the things you trust us with</div>
            ...
            <p style={{ margin: '20px 0 0', maxWidth: '64ch', fontSize: 'var(--qf-fs-19)', lineHeight: '1.6', color: '#4A4842', textWrap: 'pretty' }}>Qafilaa holds a live map of where people are and, if they filled it in, their blood group. We take that seriously. No system is perfectly secure, so here is what we actually do rather than a claim that nothing can go wrong.</p>
            ...
            <div data-legalbody="1" style={{ maxWidth: '70ch' }}>
              ...
              <p style={{ margin: '14px 0 0', fontSize: 'var(--qf-fs-17)', lineHeight: '1.72', color: '#4A4842', textWrap: 'pretty' }}>All traffic between the app and our servers runs over TLS. Position updates publish on a channel separate from application traffic, with a retained snapshot per rider so a late joiner receives the current state of the convoy on connect. Documents and images you upload — licence, RC, insurance, permits, trip photos — are stored encrypted, and the stored file is removed when you delete the record, not orphaned in a bucket.</p>
              ...
              <p style={{ margin: '14px 0 0', fontSize: 'var(--qf-fs-17)', lineHeight: '1.72', color: '#4A4842', textWrap: 'pretty' }}>Crash detection reads the accelerometer continuously while you ride. Those samples are processed on the device and are never uploaded. What reaches us is the conclusion — an alert with your position, bearing and blood group — not the raw motion stream. Your phonebook is the same: if you grant contacts access, it is read locally so you can pick an emergency contact, and the address book is not uploaded.</p>
              ...
              <p style={{ margin: '14px 0 0', fontSize: 'var(--qf-fs-17)', lineHeight: '1.72', color: '#4A4842', textWrap: 'pretty' }}>Raw location traces delete automatically after 90 days, or 7 if you shorten the window. Server logs and routine backups age out within 30 days. Account deletion runs as a single transaction — it either all goes or none of it does — so an account is never left half-erased. The detail is in <a href="/delete-account">Delete your account</a> and section 6 of the <a href="/privacy-policy">Privacy Policy</a>.</p>
